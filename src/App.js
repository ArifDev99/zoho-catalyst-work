...
const Task = forwardRef(({ id, notes, index, removeTask }, ref) => {
  ...
  const deleteTask = useCallback(() => {
    setDeleting(true);
    axios
      .delete(`/server/todo_list_function/${id}`) //Ensure that 'to_do_list_function' is the package name of your function.
      .then((response) => {
        const {
          data: {
            todoItem: { id },
...
function App() {
  ...
  useEffect(() => {
    if (fetchState !== "fetched") {
      axios
        .get("/server/todo_list_function/all", {
  ...
  const createTodo = useCallback(
    (event) => {
      ...
      setSubmitting(true);
      axios
        .post("/server/todo_list_function/add", {
          //Ensure that 'to_do_list_function' is the package name of your function.
          notes,
        })
        .then((response) => {
